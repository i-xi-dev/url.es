...
describe("Uri.prototype.originEquals", () => {
  ...
  it("originEquals(*)", () => {
    expect(() => {
      Uri.fromString("http://example.com:8080/").originEquals(1);
...
describe("Uri.prototype.withQuery", () => {
  ...
  it("withQuery(Array<*>)", () => {
    const u1 = Uri.fromString("http://example.com:80/hoge?a=1").withQuery([["b",1]]);
    expect(JSON.stringify(u1.query)).to.equal('[]');
    expect(u1.toString()).to.equal("http://example.com/hoge");

    const u1b = Uri.fromString("http://example.com:80/hoge?a=1").withQuery([["b","1","2"]]);
